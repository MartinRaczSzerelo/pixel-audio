...
if exist audio-png16.mra del audio-png16.mra
...
"ffmpeg\ffmpeg.exe" -v error -i "%input_file%" -c:a pcm_s16be -ar 48k -ac 1 -f s16be "tmp.pcm"
if errorlevel 1 (
...
set chunk_count=0
for %%f in (*.pcm) do (
    "ffmpeg\ffmpeg.exe" -v error -f rawvideo -pix_fmt gray16be -s 512x256 -i "%%f" -vf "format=gray16be" "%%~nf.png"
    if !errorlevel! equ 0 (
        del "%%f"
        set /a chunk_count+=1
        <nul set /p="."
...
"7z/7za.exe" a -tzip audio-png16.pxa *.png
del *.png
del *.pcm
